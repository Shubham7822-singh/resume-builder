...
const StepProgress = ({ progress}) => {
  return (
    ...
    border border-white/10'>
        ...
        animate-flow bg-[length:200%] transition-all duration-700 ease-out rounded-full overflow-hidden
        ...
        {[...Array(8)].map((_,i) => (
            <div key={i} className='absolute top-1/2 bg-white rounded-full animate-bubble shadow-lg'
            style={{
                left: `${(i + 1) *12 }%`,
...
: 'Nearly Completed'}
        <div className=' flex items-center gap-2'>
                    {progress === 100 && (
                        <div className=' w-6 h-6 bg-gradient-to-r from-green-500 to bg-emerald-500 rounded-full
